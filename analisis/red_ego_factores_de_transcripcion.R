...
genes_ego             <- names(V(g))[names(V(g)) %in% reguladores_de_expresion]
names(genes_ego)      <- unique(paste0(genes_ego, " (", at_simbolos[genes_ego], ")"))
if(F){
ego                   <- genes_ego[1]
g_ego                 <- make_ego_graph(g, 1, V(g)[which(names(V(g)) == ego)])[[1]]
factores_conectados   <- names(V(g_ego))[names(V(g_ego)) %in% setdiff(reguladores_de_expresion, ego)]
g_ego                 <- g_ego %u% Reduce('%u%', make_ego_graph(g, 1, V(g)[which(names(V(g)) %in% factores_conectados)]))
srp_conectados        <- names(V(g_ego))[names(V(g_ego)) %in% setdiff(proteinas_relacionadas_con_splicing, ego)]
g_ego                 <- g_ego %u% Reduce('%u%', make_ego_graph(g, 1, V(g)[which(names(V(g)) %in% srp_conectados)]))
layout_g              <- layout_nicely(g_ego)
layout_g              <- cbind(layout_g, rep(5, nrow(layout_g)))
layout_g[grep(":", names(V(g_ego))), 3] <- -5
g_ego_o               <- setGraphPlotOptions(g_ego, nombres = T)
curve_multiple(g_ego_o)
plot(g_ego_o, layout=layout_g)
rglplot(g_ego_o, layout=layout_g, vertex.shape=2)
}
#interfaz de shiny
ui <- fluidPage(
  titlePanel("Red Ego Factores de Transcripción"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      simpleNetworkOutput("red_ego", height = "700px", width = "500px"),
...
server <- function(input, output) {
  ...
  output$red_ego <- renderSimpleNetwork({
    src <- c("A", "A", "A", "A",
             "B", "B", "C", "C", "D")
    target <- c("B", "C", "D", "J",
                "E", "F", "G", "H", "I")
  ...
  #  g_interes                <- make_ego_graph(g, input$k_vecinos, V(g)[which(names(V(g)) == input$ego)])[[1]]
  ...
  output$genes_en_la_red <- renderText({ 
    #paste("Genes:", paste0(sort(intersect(rv$g_ego, names(V(g_genes)))), collapse = ", "))
  })     
  
  output$bines_en_la_red <- renderText({ 
    #paste("Bines:", paste0(sort(unique(strsplit2(intersect(rv$g_ego, names(V(g_bines))), ":")[, 1])), collapse = ", "))
  })     
